function [PD, PF, Precision, F1, AUC, MCC] = DBN_CP(sourcePath, targetPath, str_i, srcCodeDir, tarCodeDir, saveDir, idxTrainTarget)
...
%   (3) str_i       - 
%   (4) srcCodeDir  - a string, the direction of source code of source project. 
%   (5) tarCodeDir  - a string, the direction of source code of target project.
%   (6) saveDir     - a string, the direction of file save. 
%   (7) idxTrainTarget - the index of training target instances;
% OUTPUTS:
%   PD, PF, ...
%
...
% Remove training target instances
tarSem(idxTrainTarget,:)=[];
...
%% Prediction
LR = glmfit(srcDeepSem, srcSem(:,end), 'binomial', 'link', 'logit'); 
...
try
    [PD,PF,Precision,F1,AUC,MCC] = Performance(tarSem(:,end), probPos); % Call self-defined Performance()
catch
